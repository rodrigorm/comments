		...
		if ($allowAddByAuth):
			if ($isAddMode && $allowAddByAuth): ?>
				...
				if (empty($this->params[$adminRoute]) && $allowAddByAuth):
					echo $commentWidget->link(__d('comments', 'Add comment'), am($url, array('comment' => 0)));
				endif;
			endif;
		else: ?>
			<?php
				echo sprintf(__d('comments', 'If you want to post comments, you need to login first.'), $html->link(__d('comments', 'login'), array('controller' => 'users', 'action' => 'login', 'prefix' => $adminRoute, $adminRoute => false)));
		endif;
